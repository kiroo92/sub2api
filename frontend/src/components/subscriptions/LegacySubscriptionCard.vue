<template>
        <article
          :data-legacy-id="subscription.id"
          class="min-w-0 overflow-hidden rounded-lg border bg-white dark:bg-dark-800"
          :class="platformBorderClass(subscription.group?.platform || '')"
        >
          <!-- Header -->
          <div
            class="flex flex-wrap items-start justify-between gap-2 border-b border-gray-100 p-4 dark:border-dark-700"
          >
            <div class="flex min-w-0 flex-1 items-center gap-2">
              ...
              <div class="min-w-0">
                <div class="flex flex-wrap items-center gap-2">
                  ...
                  <span class="badge badge-gray">{{ t('packages.legacy') }}</span>
                  <span :class="['rounded-md border px-2 py-0.5 text-[11px] font-medium', platformBadgeClass(subscription.group?.platform || '')]">
                    {{ platformLabel(subscription.group?.platform || '') }}
                  </span>
                </div>
                <p v-if="subscription.group?.description" class="mt-0.5 break-words text-xs text-gray-500 dark:text-dark-400">
          ...
          <div class="space-y-3 p-4">
            <!-- Expiration Info -->
            <div v-if="subscription.expires_at" class="flex flex-wrap items-center justify-between gap-1 text-xs">
            ...
            <div
              v-if="
                ...
                !subscription.group?.monthly_limit_usd
              "
              class="flex items-center justify-center rounded-md bg-emerald-50 py-3 dark:bg-emerald-900/20"
            >
...
defineProps<{ subscription: UserSubscription }>()
