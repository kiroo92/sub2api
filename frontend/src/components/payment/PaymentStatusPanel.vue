    ...
    <template v-if="outcome === 'success'">
      <div class="card p-6">
        <div class="flex flex-col items-center space-y-4 py-4">
          ...
          <p class="text-lg font-bold text-gray-900 dark:text-white">{{ props.orderType === 'invoice_fee' ? t('invoices.paid') : props.orderType === 'subscription' ? t('payment.result.subscriptionSuccess') : t('payment.result.success') }}</p>
          <div v-if="paidOrder" class="w-full rounded-xl bg-gray-50 p-4 dark:bg-dark-800">
            <div class="space-y-2 text-sm">
              ...
              <div class="flex justify-between">
                <span class="text-gray-500 dark:text-gray-400">{{ t(props.orderType === 'invoice_fee' ? 'invoices.serviceFee' : 'payment.orders.amount') }}</span>
                <span class="font-medium text-gray-900 dark:text-white">{{ props.orderType === 'invoice_fee' ? formatGatewayAmount(paidOrder.amount, paidOrder.currency) : creditedAmountSymbol + paidOrder.amount.toFixed(2) }}</span>
              </div>
              <div class="flex justify-between">
