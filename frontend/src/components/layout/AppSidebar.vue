...
// 管理员的个人区也保留账户概览入口，管理仪表盘仍指向 /admin/dashboard。
//
// 条目顺序：概览 → 密钥 → 用量 → 可用渠道 → 渠道状态 → 支付 → 资料。
...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  if (withDashboard) {
    ...
    items.push({ path: '/dashboard', label: t('dashboard.accountOverview'), icon: DashboardIcon })
  }
  items.push(
    ...
    { path: '/purchase', label: t('packages.shop'), icon: RechargeSubscriptionIcon, hideInSimpleMode: true, featureFlag: flagPayment },
    { path: '/package-groups', label: t('packages.hall'), icon: UsersIcon, hideInSimpleMode: true, featureFlag: flagPayment },
...
// Personal navigation items include the user's account overview, not admin metrics.
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/lottery', label: t('lottery.adminTitle'), icon: GiftIcon, hideInSimpleMode: true },
    ...
    },
    {
      ...
      children: [
        ...
        { path: '/admin/packages', label: t('packages.admin'), icon: CreditCardIcon },
      ],
    },
