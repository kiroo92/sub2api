<template>
  <AppLayout>
    <div class="channel-monitor-theme signal-channel-page signal-channel-v3 space-y-5 pb-12">
        ...
        >
          ...
          >
            <div
              ...
              :key="`${item.platform}:${item.row.group_id ?? item.row.group_name ?? ''}`"
...
const latestSnapshotMetrics = computed(() => {
  ...
  return trend[trend.length - 1]?.metrics ?? snapshot.value?.metrics
})
