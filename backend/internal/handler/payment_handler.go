...
func applyWeChatPaymentResumeClaims(req *CreateOrderRequest, claims *service.WeChatPaymentResumeClaims) error {
	...
	if claims.CouponCode != "" {
		if req.CouponCode != "" && !strings.EqualFold(strings.TrimSpace(req.CouponCode), claims.CouponCode) {
			return infraerrors.BadRequest("INVALID_WECHAT_PAYMENT_RESUME_TOKEN", "discount resume code mismatch")
		}
		req.CouponCode = claims.CouponCode
		amount, err := strconv.ParseFloat(claims.ExpectedPayAmount, 64)
		if err != nil || amount <= 0 {
			return infraerrors.BadRequest("INVALID_WECHAT_PAYMENT_RESUME_TOKEN", "invalid discount resume amount")
		}
		req.ExpectedPayAmount = &amount
	}
	return nil
}
