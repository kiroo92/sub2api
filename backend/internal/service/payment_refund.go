...
func (s *PaymentService) PrepareRefund(ctx context.Context, oid int64, amt float64, reason string, force, deduct bool) (*RefundPlan, *RefundResult, error) {
	...
	if o.OrderType == payment.OrderTypeInvoiceFee {
		return nil, nil, infraerrors.BadRequest("INVOICE_REFUND_UNSUPPORTED", "invoice fee refunds are not supported")
	}
...
func (s *PaymentService) ExecuteRefund(ctx context.Context, p *RefundPlan) (*RefundResult, error) {
	c, err := s.entClient.PaymentOrder.Update().Where(paymentorder.IDEQ(p.OrderID), paymentorder.OrderTypeNEQ(payment.OrderTypeInvoiceFee), paymentorder.StatusIn(OrderStatusCompleted, OrderStatusRefundRequested, OrderStatusRefundPending, OrderStatusRefundFailed)).SetStatus(OrderStatusRefunding).Save(ctx)
	if err != nil {
...
func (s *PaymentService) QueryAndFinalizeRefund(ctx context.Context, oid int64) (*RefundResult, error) {
	...
	if o.OrderType == payment.OrderTypeInvoiceFee {
		return nil, infraerrors.BadRequest("INVOICE_REFUND_UNSUPPORTED", "invoice fee refunds are not supported")
	}
