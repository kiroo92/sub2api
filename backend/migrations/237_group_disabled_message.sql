-- Per-group message returned when API access is blocked by an inactive group.
ALTER TABLE groups ADD COLUMN IF NOT EXISTS disabled_message TEXT NOT NULL DEFAULT '';

-- Refresh cached messages even for out-of-band edits and interrupted admin saves.
...
BEGIN
    ...
    IF TG_OP = 'UPDATE'
       ...
       AND OLD.disabled_message IS NOT DISTINCT FROM NEW.disabled_message
